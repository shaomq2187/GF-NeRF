...
def get_new_camera_path(cameras: Cameras, idx: int = -1, scene_center: Tuple[float, ...] = (0.0, 0.0, 0.0)):
    # 随机从cameras中抽一帧
    if idx == -1:
        idx = 0
    poses = cameras.camera_to_worlds.cpu().numpy()
    # 调整scene center
    poses[:, 0, 3] -= scene_center[0]
    poses[:, 1, 3] -= scene_center[1]
    poses[:, 2, 3] -= scene_center[2]

    select_pos = poses[idx, :, 3]
    select_pose = poses[idx, ...]
    mid_pos = select_pos * 0.5
    mid_pos[2] += 0.5 * select_pos[2]

    final_pos = select_pos * 1.0
    final_pos[2] += 0.5 * select_pos[2]

    # 计算view matrix
    up = np.array([0.0, 0.0, 1.0])
    final_pose = viewmatrix(final_pos, up=up, position=final_pos)

    select_pose_2 = viewmatrix(select_pos, up=up, position=select_pos)
...
#     poses: np.ndarray, n_interp: int, spline_degree: int = 5, smoothness: float = 0.03, rot_weight: float = 0.1
# ):
...
# def get_interpolated_camera_path_spline(cameras: Cameras, steps: int) -> Cameras:
#     """Generate a camera path between two cameras.
    ...
    for camera in camera_path["camera_path"]:
        ...
        if camera_type == CameraType.EQUIRECTANGULAR:
            ...
            if "fov" in camera:
                ...
                fovx = camera['fovx']
                fovy = camera['fovy']
                focal_length_x = three_js_perspective_camera_focal_length(fovx, image_height)
                focal_length_y = three_js_perspective_camera_focal_length(fovy, image_height)
                fxs.append(focal_length_x)
                fxs.append(focal_length_y)
