...
class Trainer:
    ...
    def setup(self, test_mode: Literal["test", "val", "inference"] = "val") -> None:
        ...
        self.pipeline = self.config.pipeline.setup(
            device=self.device, test_mode=test_mode, world_size=self.world_size, local_rank=self.local_rank,base_dir = self.base_dir
        )
    ...
    def train_iteration(self, step: int) -> TRAIN_INTERATION_OUTPUT:
        ...
        # a = self.pipeline.model.field.base_encoding.feat_pool


        # print("[debug] loss value:",loss)
        # if step >= 1000:
        #     cur_split_idx = (step - 1000) // 1000
        #     self.optimizers.delete_optimizer("base_encoding_init")
        #     for i in range(cur_split_idx):
        #         self.optimizers.delete_optimizer(f"base_encoding_{i}")
        ...
        # if (step) % 1000==0:
        #     # import torchviz
        #     # vis_graph = torchviz.make_dot(loss,params=dict(self.pipeline.model.named_parameters()),show_attrs=True,show_saved=True)
        #     # # vis_graph.view()
        #     # vis_graph.render(f"graph_{step}.pdf")
        #     pass
        ...
        for tag, value in self.pipeline.model.named_parameters():
            ...
            if value.grad is not None:
                if (torch.isnan(value.grad)).any():
                    
                    print(f"[debug] {tag} get nan grad and cur loss is {loss}")
                    is_nan = True
