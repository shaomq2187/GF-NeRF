...
@dataclass
class GFNerfExponentialDecaySchedulerConfig(SchedulerConfig):
    """Config for exponential decay scheduler with warmup"""

    _target: Type = field(default_factory=lambda: GFNerfExponentialDecayScheduler)
    """target class to instantiate"""
    lr_pre_warmup: float = 1e-8
    """Learning rate before warmup."""
    lr_final: Optional[float] = None
    ...
    n_dataset_circles: int = 1

    steps_per_split_dataset: int = 1000

    steps_perssampler_init: int = 10000

class GFNerfExponentialDecayScheduler(Scheduler):
    """
        Exponential decay scheduler for GFNerf
    """

    config: GFNerfExponentialDecaySchedulerConfig
    ...
    def get_scheduler(self, optimizer: Optimizer, lr_init: float) -> lr_scheduler._LRScheduler:
        ...
        def func(step):
            if step < self.config.warmup_steps:
                ...
                cur_stage = 'init_stage' if (self.config.steps_perssampler_init >0 and step < self.config.steps_perssampler_init) else 'block_stage'
                if cur_stage == 'init_stage':
                    relative_step = step
                else:
                    cur_split_dataset_idx = ((step - self.config.steps_perssampler_init) // self.config.steps_per_split_dataset) % self.config.n_split_dataset
                    cur_circles = (step - self.config.steps_perssampler_init) // (self.config.steps_per_split_dataset * self.config.n_split_dataset) 
                    relative_step = step - self.config.steps_perssampler_init -\
                        cur_circles * self.config.steps_per_split_dataset * self.config.n_split_dataset -\
                        cur_split_dataset_idx * self.config.steps_per_split_dataset + cur_circles * self.config.steps_per_split_dataset


                t = np.clip(
                    (relative_step - self.config.warmup_steps) / (self.config.max_steps - self.config.warmup_steps), 0, 1
                )
                lr = np.exp(np.log(lr_init) * (1 - t) + np.log(lr_final) * t)
