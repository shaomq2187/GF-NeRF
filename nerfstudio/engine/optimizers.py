...
class Optimizers:
    ...
    def __init__(self, config: Dict[str, Any], param_groups: Dict[str, List[Parameter]]) -> None:
        ...
        for param_group_name, params in param_groups.items():
            if param_group_name not in config:
                config[param_group_name] = config['fields']
    ...
    def scheduler_step_all(self, step: int) -> None:
        ...
        for param_group_name, scheduler in self.schedulers.items():
            print('[scheduler] param_group_name:',param_group_name)
            scheduler.step()
    ...
    def delete_optimizer(self,param_group_name):
        # delete param group to save memory
        if param_group_name in self.optimizers:
            del self.optimizers[param_group_name]
            del self.schedulers[param_group_name]
            del self.parameters[param_group_name]
            del self.param_groups[param_group_name]
            torch.cuda.empty_cache()

    def add_optimizer(self,param_group_name_target,params = None,lr_init = None):
        # add param group from param_group_name_target
        if param_group_name_target not in self.optimizers:

            if params is not None and param_group_name_target not in self.param_groups.keys():
                self.param_groups[param_group_name_target] = params
            
                
            for param_group_name, params in self.param_groups.items():
                if param_group_name_target == param_group_name:
                    if param_group_name not in self.config:
                        self.config[param_group_name] = self.config['fields']
                    if lr_init is None:
                        lr_init = self.config[param_group_name]["optimizer"].lr
                    else:
                        self.config[param_group_name]["optimizer"].lr = lr_init
                    self.optimizers[param_group_name] = self.config[param_group_name]["optimizer"].setup(params=params)
                    self.parameters[param_group_name] = params
                    if self.config[param_group_name]["scheduler"]:
                        self.schedulers[param_group_name] = (
                            self.config[param_group_name]["scheduler"]
                            .setup()
                            .get_scheduler(optimizer=self.optimizers[param_group_name], lr_init=lr_init)
                        )
