...
# from torch._six import string_classes   work on torch 1.12.0, higher version is not compatible
string_classes = str # for higher torch version
...
def nerfstudio_collate(
    ...
    if isinstance(elem, torch.Tensor):  # pylint: disable=no-else-return
        ...
        try:
            return torch.stack(batch, 0, out=out)  #  TODO: crash here 
        except RuntimeError:            
            return batch
