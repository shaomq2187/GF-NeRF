...
class SceneContraction(SpatialDistortion):
    ...
    def forward(self, positions):
        ...
        if isinstance(positions, Gaussians):
            ...
            jc_means = torch.vmap(jacrev(contract))(positions.mean.view(-1, positions.mean.shape[-1]))
            ...
            # cov[mask] = ((jc_means[mask] @ positions.cov[mask]).float() @ torch.transpose(jc_means[mask], -2, -1)).float()
