...
def run_colmap(
    ...
    print(str(image_dir).replace("images","masks"))
    if os.path.exists(str(image_dir).replace("images","masks")):
        mask_path = str(image_dir).replace("images","masks")
        feature_extractor_cmd.append(f"--ImageReader.mask_path {mask_path}")
        print("!!!!![debug] mask is uesed in colmap!!!!!")
...
def colmap_to_json(
    ...
    ## 得到相机内参，以第一个相机为基本内参，写在out中
    out = parse_colmap_camera_params(cam_id_to_camera[1])

    ## 得到所有相机内参，得到不同camera_id对应的内参
    camera_id_camera_info_dict = {}
    for camera_id in cam_id_to_camera:
        camera_info = parse_colmap_camera_params(cam_id_to_camera[camera_id])
        camera_id_camera_info_dict[camera_id] = camera_info
    ...
    for im_id, im_data in im_id_to_image.items():
        ...
        camera_info = camera_id_camera_info_dict[camera_id]
