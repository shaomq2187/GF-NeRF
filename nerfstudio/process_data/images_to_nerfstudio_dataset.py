...
class ImagesToNerfstudioDataset(ColmapConverterToNerfstudioDataset):
    ...
    def main(self) -> None:
        ...
        if not self.skip_image_processing:
            ...
            num_frames = len(process_data_utils.list_images_per_folder(self.data))
            if num_frames == 0:
                raise RuntimeError("No usable images in the data folder.")
            summary_log.append(f"Starting with {num_frames} images")

        # Check if colmap images is empty
        if len(process_data_utils.list_images_per_folder(self.image_dir)) == 0:
            process_data_utils.copy_image_per_folder(self.data, self.image_dir)
