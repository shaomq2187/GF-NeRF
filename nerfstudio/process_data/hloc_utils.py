...
def run_hloc(
    ...
    references = [path.relative_to(image_dir).as_posix() for path in image_dir.glob("**/*") if path.is_file()]
    ...
    if refine_pixsfm:
        ...
        refined, _ = sfm.reconstruction(
            ...
            camera_mode=pycolmap.CameraMode.PER_FOLDER,  # 这里可以设置成per folder，但前面的处理需要进行
