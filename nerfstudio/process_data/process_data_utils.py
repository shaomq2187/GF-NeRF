...
def get_numerical_value(path):
    """从路径中提取数字值"""
    # 从路径的最后一部分（文件名）提取数字部分
    filename = path.name
...
def list_images_per_folder(data: Path) -> List[Path]:
    ...
    image_paths = sorted(image_paths, key=get_numerical_value)
    return image_paths
...
def list_images(data: Path) -> List[Path]:
    ...
    image_paths = sorted([p for p in data.glob("[!.]*") if p.suffix.lower() in allowed_exts], key=get_numerical_value)
    return image_paths
...
def copy_image_per_folder(source_dir: Path, dest_dir: Path):
    """Copy all images from source directory to destination directory, including subdirectories.
...
def scan_image_per_folder(directory):
    image_files = []
...
def downscale_images_per_folder(
    image_dir: Path,
    num_downscales: int,
    folder_name: str = "images",
    nearest_neighbor: bool = False,
    ...
    with status(msg="[bold yellow]Downscaling images...", spinner="growVertical", verbose=verbose):
        ...
        for downscale_factor in downscale_factors:
            ...
            # 字符串替换，保证子目录结构
            # downscale_dir = image_dir.parent / f"{folder_name}_{downscale_factor}"
            downscale_dir = Path(str(image_dir).replace("images", f"images_{downscale_factor}"))
            downscale_dir.mkdir(parents=True, exist_ok=True)
            # Using %05d ffmpeg commands appears to be unreliable (skips images), so use scandir.
            # files = os.scandir(image_dir)
            # 使用自定义函数搜索给定目录下所有图片，包括子目录
            files = scan_image_per_folder(image_dir)
            for f in files:
                if f.is_dir():
                    continue
                filename = f.relative_to(image_dir)
                nn_flag = "" if not nearest_neighbor else ":flags=neighbor"

                # 图片的父母录不是images_x的话，说明有子目录存在，创建子目录
                if filename.parent != downscale_dir.name:
                    Path(downscale_dir / filename.parent).mkdir(parents=True, exist_ok=True)
                ffmpeg_cmd = [
                    f'ffmpeg -y -noautorotate -i "{image_dir / filename}" ',
                    f"-q:v 2 -vf scale=iw/{downscale_factor}:ih/{downscale_factor}{nn_flag} ",
                    f'"{downscale_dir / filename}"',
