...
__global__ void Hash3DAnchoredForwardKernel(int n_points, int n_volumes,
                                            ...
                                            Wec3f* bias_pool,
                                            Wec3f* points_ptr, int64_t* volume_idx,
                                            T* out_feat) {
  ...
  {
    const int64_t offset = (level_idx * n_volumes + volume_idx[0]) * 3;
  ...
  int64_t transf_idx = level_idx * n_volumes + volume_idx[0];
...
variable_list Hash3DAnchoredFunction::forward(AutogradContext* ctx,
  ...
  Hash3DAnchoredForwardKernel<FlexType><<<grid_dim, block_dim>>>(
      ...
      RE_INTER(Wec3f*, points.data_ptr()), volume_idx.data_ptr<int64_t>(),
...
variable_list Hash3DAnchoredFunction::backward(AutogradContext* ctx, variable_list grad_output) {
  ...
  // std::cout<<"grad_in max abs:"<<grad_in<<std::endl;

  Tensor true_grad_out = torch::zeros({ pool_size,  N_CHANNELS }, CUDAFlex);
  // std::cout<<"feat_local_idx:"<<feat_local_idx<<std::endl;
  // std::cout<<"feat_local_size:"<<feat_local_size<<std::endl;
  // std::cout<<"volume_idx:"<<volume_idx<<std::endl;
  ...
  Hash3DAnchoredBackwardKernel<FlexType><<<grid_dim, block_dim>>>(
    ...
    // std::cout<<"points abs:"<<points<<std::endl;

  // std::cout<<"trur_grad_out max abs:"<<torch::max(torch::abs(true_grad_out))<<std::endl;
